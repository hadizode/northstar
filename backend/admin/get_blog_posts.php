...
foreach ($posts as &$post) {
    ...
    $comments = db()->prepare('
        SELECT c.*,
               (SELECT COUNT(*) FROM blog_comments r WHERE r.parent_id = c.id AND r.is_approved = 1) as reply_count
        FROM blog_comments c
        WHERE c.post_id = :post_id AND c.is_approved = 1 AND c.parent_id IS NULL
